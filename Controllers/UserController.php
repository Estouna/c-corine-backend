...
class UserController extends Controller
{
    ...
    */
    ...
    public function login()
    {
        ...
        $form->debutForm('post', '#', ['class' => 'form column'])
            ->ajoutLabelFor('email', 'E-mail :', ['class' => 'form__label'])
            ->ajoutInput('email', 'email', ['class' => 'form__input', 'id' => 'email', 'required' => 'true'])
            ->ajoutLabelFor('password', 'Mot de passe :', ['class' => 'form__label'])
            ->ajoutInput('password', 'password', ['class' => 'form__input', 'id' => 'password', 'required' => 'true'])
            ->debutDiv(['class' => 'text-center form__div--validate'])
            ->ajoutBouton('Me connecter', ['type' => 'submit', 'name' => 'validateLog', 'class' => 'form__button--validate'])
            ->finDiv()
            ->finForm();

        // Envoi le formulaire à la vue
        $this->render('user/login', ['loginForm' => $form->create()], 'login-register');
    }
    ...
    */
    ...
    public function register()
    {
        ...
        // Formulaire
        $form->debutForm('post', '#', ['class' => 'form column'])
            ->ajoutLabelFor('pseudo', 'Pseudo :', ['class' => 'form__label'])
            ->ajoutInput('text', 'pseudo', ['class' => 'form__input', 'id' => 'pseudo', 'minlength' => '1', 'maxlength' => '20', 'required' => 'true'])
            ->ajoutLabelFor('email', 'E-mail :', ['class' => 'form__label'])
            ->ajoutInput('email', 'email', ['class' => 'form__input', 'id' => 'email', 'required' => 'true'])
            ->ajoutLabelFor('pass', 'Mot de passe :', ['class' => 'form__label'])
            ->ajoutInput('password', 'password', ['class' => 'form__input', 'id' => 'pass', 'required' => 'true'])
            ->debutDiv(['class' => 'text-center form__div--validate'])
            ->ajoutBouton('M\'inscrire', ['type' => 'submit', 'name' => 'validateReg', 'class' => 'form__button--validate'])
            ->finDiv()
            ->finForm();

        // Envoi le formulaire à la vue
        $this->render('user/register', ['registerForm' => $form->create()], 'login-register');
    }
